{
    ...
    const triangle = makeMovablePolygon([{
        ...
        const c = calculateDistanceBetweenTwoPoints(coords[0], coords[1])
        const b = calculateDistanceBetweenTwoPoints(coords[0], coords[2])
        const a = calculateDistanceBetweenTwoPoints(coords[1], coords[2])

        const x=2.0*b*c/(b*b+c*c-a*a)
        const y=2.0*a*c/(a*a+c*c-b*b)
