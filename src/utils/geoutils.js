...
export const distToLine = (p, v, w) => {
  const l2 = findDistance(v, w);
  if (l2 === 0) return findDistance(p, v);
  const t = ((p[0] - v[0]) * (w[0] - v[0]) + (p[1] - v[1]) * (w[1] - v[1])) / l2;

  return Math.sqrt(findDistance(p, [v[0] + t * (w[0] - v[0]), v[1] + t * (w[1] - v[1])]));
};

export const pointAlongLine = (A, B, d) => {
...
export const projectPoint2Line = (p, a, b) => {
...
export const drawFullLine = (pt, m, line) => {
  if (isNaN(m)) {
    m = 0;
  } else if (!isFinite(m)) {
...
export const ptoPointRad = (theta, rad) => {
  return [rad * Math.cos(theta), -rad * Math.sin(theta)];
};

export const drawLine = (a, b, l) => {
...
export const makeSvgTriangle = (pt0, pt1, pt2, ptoTri) => {
  ptoTri.setAttributeNS(null, "points", makeString([pt0, pt1, pt2]));
};

export const makeSvgCircle = (r, x, y) => {
  ptoCircle.setAttributeNS(r, x, y);
};

export const getEquationFromPoint = (a, b) => {
  const m = (a[1] - b[1]) / (a[0] - b[0]);
  const bb = a[1] - m * a[0];
  return [m, bb];
};

export const pedalPoint = (a, b, c, p) => {
