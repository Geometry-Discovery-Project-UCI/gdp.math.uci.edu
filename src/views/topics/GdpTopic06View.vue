    ...
    <ATabPane key="2" tab="Pascal">
      <div id="Pascal-wrapper" class="animation-layout">
        ...
        <div>
          <ATypographyParagraph>
            <p>
              Pascal's Theorem is named after Blaise Pascal, which was formulated by Blaise Pascal
              in a note written in 1639 when he was only 16 years old.
            </p>
            <p>
              Pascal's Theorem states that if <span v-katex>ABCDEF</span> is an inscribed hexagon of
              a circle, then the intersections <span v-katex>X, Y, Z</span> of the three pairs of
              the opposite sides of the hexagon is collinear.
            </p>
            <p>
              Click, hold, and move any of points <span v-katex>A, B, C, D, E, F</span> to observe
              that points <span v-katex>X, Y, Z</span> are always collinear.
            </p>
            <p>
              Pascal's Theorem can be generalized to the case when a hexagon is inscribed in a conic
              curve. Since two lines can be regarded as a degenerated conic curve, Pappus' Theorem
              is a special case of the (generalized) Pascal Theorem.
            </p>
          </ATypographyParagraph>
        </div>
      </div>
    ...
    <ATabPane key="3" tab="Desargues">
      <div id="Desargues-wrapper">
        <div>
          <ATypographyParagraph>
            <p>
              Two triangles in a plane are perspective from a point <span v-katex>O</span>, called
              the center of perspectivity, if the lines joining corresponding vertices of the
              triangles meet at <span v-katex>O</span>. Dually, the triangles are said to be
              perspective from a line if the points of intersection of corresponding lines all lie
              on one line.
            </p>
            <p>
              Desargues' theorem, named after Girard Desargues, states that two triangles are
              perspective from a point, if and only if it is perspective from a point.
            </p>
            <p>
              In the following picture, assume that <span v-katex>AA', BB', CC'</span> are
              concurrent. Let <span v-katex>P,Q,R</span> be the intersections of
              <span v-katex>BC</span> and <span v-katex>B'C'</span>; <span v-katex>CA</span> and
              <span v-katex>C'A'</span>; <span v-katex>AB</span> and <span v-katex>A'B'</span>,
              respectively. Then
              <span v-katex>P, Q, R</span>
              are collinear.
            </p>
            <p>
              Click, hold, and move any of points <span v-katex>A, B, C </span> to observe that
              points <span v-katex>P,Q, R</span> are always collinear.
            </p>
          </ATypographyParagraph>
        </div>
        <div>
    ...
    <ATabPane key="4" tab="Brainchon">
      <div id="Brainchon-wrapper" class="animation-layout">
        ...
        <div>
          <ATypographyParagraph>
            <p>
              Brainchon's Theorem is named after French mathematician Charles Julien Brianchon. It
              is the famous dual theorem of Pascal's Theorem. Like Pascal's Theorem, it can also be
              generalized to the case of conic curves.
            </p>
            <p>
              Brainchon's Theorem states that if <span v-katex>ABCDEF</span> is a hexagon
              circumscribed a circle, then the main diagonals <span v-katex>AD, BE, CF</span>
              are concurrent.
            </p>
            <p>
              Click, hold, and move any of points <span v-katex>A, B, C, D, E, F</span> to observe
              that lines <span v-katex>AD, BE, CF</span> are always concurrent. The Hexagon
              <span v-katex>ABCDEF</span> doesn't have to be convex for the Brianchon's Theorem to
              hold.
            </p>
          </ATypographyParagraph>
        </div>
...
export default defineComponent({
  setup() {
    ...
    function partOne() {
      ...
      const pY = (fabric.Intersection.intersectLineLine(pA, ppC, ppA, pC) as Intersection)
        .points![0];
      const pX = (fabric.Intersection.intersectLineLine(pB, ppC, pC, ppB) as Intersection)
        .points![0];
    ...
    function partThree() {
      ...
      const primeTriangle = new fabric.Polygon(
        circleToCoord(pointPrimeA, pointPrimeB, pointPrimeC),
        {
          fill: "#5efc03",
        }
      );
      ...
      const onPointMove = (e: IEvent): void => {
        ...
        Object.assign(
          pq,
          solveLinearEquation(
            { x: pq.p1!.left!, y: pq.p1!.top! },
            { x: pq.p2!.left!, y: pq.p2!.top! }
          )
        );
        pq.set({
          x1: 0,
    ...
    function partFour() {
      ...
      ab.tan = makeCircle(
        new fabric.Point(pa.left!, pa.top!).lerp({ x: pb.left!, y: pb.top! }, 0.5)
      );
      bc.tan = makeCircle(
        new fabric.Point(pc.left!, pc.top!).lerp({ x: pb.left!, y: pb.top! }, 0.5)
      );
      cd.tan = makeCircle(
        new fabric.Point(pc.left!, pc.top!).lerp({ x: pd.left!, y: pd.top! }, 0.5)
      );
      de.tan = makeCircle(
        new fabric.Point(pd.left!, pd.top!).lerp({ x: pe.left!, y: pe.top! }, 0.5)
      );
      ef.tan = makeCircle(
        new fabric.Point(pe.left!, pe.top!).lerp({ x: pf.left!, y: pf.top! }, 0.5)
      );
      fa.tan = makeCircle(
        new fabric.Point(pa.left!, pa.top!).lerp({ x: pf.left!, y: pf.top! }, 0.5)
      );
      // Tangent lines
      ...
      state.fouthCanvas = true;
    }

    onMounted(() => {
      handleTabChange(state.activeKey1);
    });
