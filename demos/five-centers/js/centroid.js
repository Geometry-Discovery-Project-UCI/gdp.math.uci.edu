{
    const canvas = new fabric.Canvas("centroid-canvas", { selection: false });
    ...
    const triangle = makeMovablePolygon([{
        ...
        dLabel.set({ left: mp1.x - 25, top: mp1.y - 15 });
        eLabel.set({ left: mp2.x + 15, top: mp2.y - 15 });
        fLabel.set({ left: mp3.x, top: mp3.y });

        const intersect = calculateIntersect(median1, median2);

        gLabel.set({ left: intersect.x + 5, top: intersect.y + 10 });
    });
